...
class Rcon(Steam):
    ...
    def is_rcon_enabled(self, instance_name=None):
        ...
        return (
            i_config["rcon_port"] is not None and i_config["rcon_password"] is not None
        )

    def _get_rcon_args(self, instance_name=None):
        i_config = self.config.get_instance_config(instance_name)
        args = {
            "address": (i_config["steam_query_ip"], i_config["rcon_port"]),
    ...
    def _prestop(self, seconds_to_stop, is_restart):
        if self.config["say_command"] is not None and self.is_rcon_enabled(
            self.config["current_instance"]
        ):
    ...
    def _stop(self):
        if self.config["stop_command"] is not None and self.is_rcon_enabled(
            self.config["current_instance"]
        ):
            self.invoke(
    ...
    def command(self, command_string, do_print=True, *args, **kwargs):
        ...
        if self.is_running(instance):
            if self.is_rcon_enabled(instance):
                ...
                if do_print:
                    self.logger.warning(
                        "{} does not have RCON enabled".format(i_config["name"])
                    )
    ...
    @multi_instance
    @click.command()
    @click.option("-vc", "--save_command", type=str, help="Command to save the server")
    @click.pass_obj
    def save(self, do_print=True, *args, **kwargs):
        ...
        return self.invoke(
            self.command, command_string=i_config["save_command"], do_print=do_print
        )
    ...
    def shell(self, *args, **kwargs):
        ...
        if self.is_running(self.config["current_instance"]):
            ...
            raise click.ClickException("{} is not running".format(self.config["name"]))
