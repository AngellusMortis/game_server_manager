...
class Steam(Base):
    ...
    def _parse_line(self, bar, line):
        ...
        self.logger.debug("processed: {}: {} / {}".format(step_name, current, total))
        if bar is None and current < total:
            bar = click.progressbar(
                length=total, show_eta=False, show_percent=True, label=step_name
            )
        if bar is not None:
            bar.update(current)
    ...
    def _wait_until_validated(self, process, detailed_status=False):
        if detailed_status:
            ...
            buffer = Queue()
            thread = Thread(
                target=_enqueue_output, args=(process.stdout, buffer), daemon=True,
            )
            thread.start()
            ...
            self.logger.info(
                "validating {}...".format(self.config["app_id"]), nl=False,
            )
        ...
        if self._servers.get(instance_name) is None:
            ...
            if i_config is not None:
                self._servers[instance_name] = ServerQuerier(
                    (i_config["steam_query_ip"], i_config["steam_query_port"],)
                )
    ...
    def status(self, *args, **kwargs):
        ...
        if self.is_running(instance):
            ...
            try:
                ...
                self.logger.info("server name: {}".format(server_info["server_name"]))
                ...
                self.logger.info("server type: {}".format(server_info["server_type"]))
                self.logger.info(
                    "password protected: {}".format(server_info["password_protected"])
                )
                self.logger.info("VAC: {}".format(server_info["vac_enabled"]))
                self.logger.info("version: {}".format(server_info["version"]))
            except NoResponseError:
                self.logger.error(
                    "{} is running but not accesible".format(i_config["name"])
                )
        else:
    ...
    def workshop_download(self, *args, **kwargs):
        ...
        status = self.invoke(self.install, app_id=self.config["workshop_id"],)
            ...
            for workshop_item in bar:
                try:
                    self.run_command(
                        ...
                        "+workshop_download_item {} {} +quit".format(
                            self.config["steamcmd_path"],
                            self.config["path"],
