...
STEAM_DOWNLOAD_URL = (
    "https://steamcdn-a.akamaihd.net/client/installer/steamcmd_linux.tar.gz"  # noqa
)
...
class Ark(Rcon):
    ...
    def __init__(self, *args, **kwargs):
        super(Ark, self).__init__(supports_multi_instance=True, *args, **kwargs)
    ...
    def get_ark_config(self, instance_name=None):
        ...
        ark_config["params"]["ServerAdminPassword"] = i_config[
            "rcon_password"
        ] or ark_config["params"].get("ServerAdminPassword")

        if len(i_config["workshop_items"]) > 0:
            ark_config["params"]["GameModIds"] = ",".join(
                [str(i) for i in i_config["workshop_items"]]
            )
        ...
        for root, dirs, files in os.walk(mod_dir):
            for filename in files:
                ...
                if u_size == size:
                    ...
                    self.logger.error("could not validate {}".format(to_extract_path))
                    return False
        return True
    ...
    @click.option("-am", "--ark_map", type=str, help="Map to initalize ARK server with")
    @click.option(
        "-ap",
        "--ark_params",
        type=str,
    ...
    def start(self, no_verify, *args, **kwargs):
        ...
        server_command = os.path.join(
            self.config["path"], "ShooterGame", "Binaries", "Linux", "ShooterGameServer"
        )

        command = (
            "{} {} -server -servergamelog -log " "-servergamelogincludetribelogs"
        ...
        return self.invoke(
            super(Ark, self).start, no_verify=no_verify, command=command,
        )
    ...
    def install(self, *args, **kwargs):
        ...
        status = self.invoke(super(Ark, self).install)
    ...
    def workshop_download(self, *args, **kwargs):
        ...
        status = self.invoke(super(Ark, self).workshop_download)
            ...
            with click.progressbar(self.config["workshop_items"]) as bar:
                for workshop_item in bar:
                    ...
                    branch_dir = os.path.join(
                        src_dir, "{}NoEditor".format(self.config["workshop_branch"])
                    )
                    mod_dir = os.path.join(mod_path, str(workshop_item))
                    mod_file = os.path.join(mod_path, "{}.mod".format(workshop_item))
                    ...
                    if os.path.isdir(mod_dir):
                        self.logger.debug(
                            "removing old mod_dir of {}...".format(workshop_item)
                        )
                        shutil.rmtree(mod_dir)
                    if os.path.isfile(mod_file):
                        self.logger.debug(
                            "removing old mod_file of {}...".format(workshop_item)
                        )
                        os.remove(mod_file)
                    ...
                    if not self._create_mod_file(mod_dir, mod_file, workshop_item):
                        self.logger.error(
                            "could not create .mod file for {}".format(workshop_item)
                        )
                        return STATUS_FAILED
