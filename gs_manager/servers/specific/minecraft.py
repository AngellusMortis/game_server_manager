...
class Minecraft(Java):
    ...
    def mc_config(self):
        if self._mc_config is None:
            ...
            config_path = os.path.join(self.config["path"], "server.properties")
        ...
        with click_spinner.spinner():
            while processing:
                for line in tail.readlines():
                    ...
                    done_match = (
                        re.search("Done \((\d+\.\d+)s\)! For help,", line) is not None
                    )
                    if done_match:
                        processing = False
                ...
                if loops_since_check < 5:
                    ...
                    self.logger.error("{} failed to start".format(self.config["name"]))
                    processing = False
                time.sleep(1)
        if os.path.isfile(offset_file):
            os.remove(offset_file)
    ...
    def _startup_check(self, instance_name=None):
        ...
        if os.path.isfile(log_file):
            if self._process_log_file(log_file, instance_name):
                ...
                return super(Minecraft, self)._startup_check(instance_name)
        else:
    ...
    def status(self, detailed, *args, **kwargs):
        ...
        if self.is_running(self.config["current_instance"]):
            if detailed and not self.mc_config.get("enable-query") == "true":
                raise click.ClickException("query is not enabled in server.properties")
            ...
            try:
                ...
                self.logger.error(
                    "{} is running, but not accessible".format(self.config["name"])
                )
                return STATUS_FAILED
            else:
                self.logger.success("{} is running".format(self.config["name"]))
                if query is not None:
                    self.logger.info(
                        "host: {}:{}".format(
                            query.raw["hostip"], query.raw["hostport"],
                        )
                    )
                    self.logger.info(
                        "software: v{} {}".format(
                            query.software.version, query.software.brand,
                        )
                    )
                self.logger.info(
                ...
                self.logger.info('description: "{}"'.format(status.description,))
                if query is not None:
                    self.logger.info("plugins: {}".format(query.software.plugins))
                    self.logger.info('motd: "{}"'.format(query.motd))

                self.logger.info(
                    "players: {}/{}".format(status.players.online, status.players.max,)
                )
    ...
    @multi_instance
    @click.command()
    @click.option("-n", "--no_verify", is_flag=True)
    @click.option(
        "-hc",
        "--history",
        type=int,
    ...
    @click.option(
        "-sm", "--start_memory", type=int, help="Starting amount of member (in MB)"
    )
    @click.option("-mm", "--max_memory", type=int, help="Max amount of member (in MB)")
    @click.option(
        "-tc", "--thread_count", type=int, help="Number of Garbage Collection Threads"
    )
    @click.option(
        "-sj", "--server_jar", type=click.Path(), help="Path to Minecraft server jar"
    )
    @click.option(
        "-jp", "--java_path", type=click.Path(), help="Path to Java executable"
    )
    @click.option(
        "-fg",
        "--foreground",
    ...
    @click.option(
        ...
        "server. Be sure to read {} before accepting".format(EULA_URL),
    )
    @click.pass_obj
    def start(self, no_verify, accept_eula, *args, **kwargs):
        ...
        if self.config["add_property"] or self.config["remove_property"]:
            properties = validate_key_value(None, None, self.config["add_property"])
            self.mc_config.update(properties)

            for server_property in self.config["remove_property"]:
                del self.mc_config[server_property]

            property_path = os.path.join(self.config["path"], "server.properties")
        ...
        return self.invoke(
            super(Minecraft, self).start, java_args=java_args, no_verify=no_verify,
        )
    ...
    @click.option("-b", "--beta", is_flag=True)
    ...
    def install(self, force, beta, enable, minecraft_version, *args, **kwargs):
        ...
        if os.path.isdir(jar_dir):
            if os.path.isfile(jar_path):
                if force:
                    ...
                    raise click.BadParameter(
                        "minecraft v{} already installed".format(minecraft_version),
                        self.context,
                        get_param_obj(self.context, "minecraft_version"),
                    )
        else:
        ...
        self.logger.success("minecraft v{} installed".format(minecraft_version))

        link_path = os.path.join(self.config["path"], "minecraft_server.jar")
        if not os.path.isfile(link_path) or enable:
            return self.invoke(self.enable, minecraft_version=minecraft_version,)
        return STATUS_SUCCESS
    ...
    def enable(self, force, minecraft_version, *args, **kwargs):
        ...
        if self.is_running(self.config["current_instance"]):
            self.logger.error("{} is still running".format(self.config["name"]))
            return STATUS_FAILED
        ...
        if not (os.path.islink(link_path) or force or not os.path.isfile(link_path)):
            raise click.ClickException(
                "minecraft_server.jar is not a symbolic link, " "use -f to override"
            )
