...
def _create_pipeline(args, previous_process=None, redirect_output=True, **kwargs):
    processes = []
    ...
    processes.append(subprocess.Popen(args1, **kwargs))
    ...
    if "|" in args2:
        ...
        processes.append(subprocess.Popen(args[split_index + 1 :], **kwargs))
...
def run_command(command, redirect_output=True, return_process=False, **kwargs):
    ...
    processes = []
    if "|" in args:
        processes = _create_pipeline(args, redirect_output=redirect_output, **kwargs)
    ...
    if return_process:
        ...
        raise subprocess.CalledProcessError(processes[-1].returncode, command, stdout)
